...
class StoryViewState extends State<StoryView> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    var item = widget.storyItems.firstWhereOrNull((it) => !it!.shown);
    item ??= widget.storyItems.last;
    var isMyStory = item?.story?.isOwnedByCurrentActor() == true;
              ...
              return Expanded(
                child: Stack(
                  children: <Widget>[
                    ...
                    SafeArea(
                      child: widget.overlayWidget != null && item != null
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                ...
                                return GestureDetector(
                                  onTap: () async {
                                    ...
                                    sendReply(
                                        reply: storyQuickReplyEmojis[index],
                                        item: item,
                                        isEmoji: true);
                                  },
                        ...
                        Expanded(
                          child: Container(
                            ...
                            decoration: ShapeDecoration(
                              shape: StadiumBorder(
                                  side: BorderSide(
                                      color: cWhite.withValues(alpha: 0.6),
                                      width: 1)),
                            ),
                            height: 40,
                            width: double.infinity,
                            ...
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                hintStyle: MyTextStyle.gilroyRegular(
                                    color: cLightText.withValues(alpha: 0.8)),
                              ...
                              onSubmitted: (value) {
                                sendReply(
                                    reply: value, item: item, isEmoji: false);
                              },
                            ),
                          ),
                        ),
  ...
  void sendReply(
      {StoryItem? item, required String reply, required bool isEmoji}) {
    if (reply.isEmpty) return;
    var chattingController =
        ChattingController(user: item?.story?.user, shouldCallAPI: false);
    if (item?.story != null) {
      HapticFeedback.mediumImpact();
