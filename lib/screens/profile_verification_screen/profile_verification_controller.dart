...
class ProfileVerificationController extends ProfilePictureController {
  ...
  List<SettingCommon> types =
      SessionManager.shared.getSettings()?.documentType ?? [];
  List<VerificationMethodModel> methods = [
    VerificationMethodModel(LKeys.verificationSubscriptionTitle,
        LKeys.verificationSubscriptionDesc, VerificationMethod.subscription),
    VerificationMethodModel(LKeys.verificationDocumentTitle,
        LKeys.verificationDocumentDesc, VerificationMethod.document),
  ];
  ...
  void setupSubscriptions() {
    if (selectedPackage == null || packages.isEmpty) {
      ...
      if (SubscriptionManager.shared.packages.isNotEmpty) {
  ...
  void restorePurchase() async {
    startLoading();
    final (customerInfo, _) =
        await SubscriptionManager.shared.restorePurchase();
    if (customerInfo != null &&
        SubscriptionManager.shared.checkSubscription(customerInfo)) {
  ...
  void submitRequest() async {
    if (selectedMethod.method == VerificationMethod.document) {
      if (fullNameController.text.isEmpty) {
        ...
        startLoading();
        UserService.shared.profileVerification(fullNameController.text,
            selectedType?.title ?? '', selectedDocument, XFile(imagePath));
      }
    } else {
      if (selectedPackage != null) {
        startLoading();
        var status =
            await SubscriptionManager.shared.makePurchase(selectedPackage!);
        if (status == true) {
          UserService.shared.editProfile(
            isVerified: 3,
            completion: (p0) {
        ...
        showSnackBar(LKeys.pleaseConfigureSubscription.tr,
