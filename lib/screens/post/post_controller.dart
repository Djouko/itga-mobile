...
import 'package:untitled/screens/post/edit_post_sheet.dart';
import 'package:untitled/screens/post/post_liked_users_screen.dart';
import 'package:untitled/screens/post/post_reposted_users_screen.dart';
...
class PostController extends BaseController {
  ...
  bool get isCurrentActorOwner {
    final companyId = SessionManager.shared.getActingCompanyId();
    if (companyId != null) {
      return post.companyId == companyId;
    }
    return post.userId == SessionManager.shared.getUserID() &&
        post.companyId == null;
  }
  ...
  void onSaved() {
    ...
    MyDebouncer.shared.run(() async {
      UserService.shared.editProfile(
          savedPostIds: SessionManager.shared.getUser()?.getSavedPostIdsList(),
          completion: (p0) {});
    });
  }
  ...
  void saveEditedPost({required String desc}) {
    startLoading();
    PostService.shared.editPost(
      ...
      completion: (updatedPost) {
        ...
        update();
        refreshView();
        showSnackBar(LKeys.postUpdatedSuccessfully.tr,
            type: SnackBarType.success);
      },
    );
  }

  void showWhoRepostedThePost() {
    Future.delayed(const Duration(milliseconds: 1), () {
      Get.to(() => PostRepostedUsersScreen(postId: post.id ?? 0));
    });
  }

  void sharePost() {
    ShareManager.shared
        .shareTheContent(key: ShareKeys.post, value: post.id ?? 0);
  }

  void repostPost({String? desc}) {
    HapticManager.shared.light();
    if (isCurrentActorOwner) {
      showSnackBar(LKeys.cannotRepostOwnPost.tr, type: SnackBarType.error);
      return;
    }
    startLoading();
    PostService.shared.repostPost(
      postId: post.id ?? 0,
      desc: desc,
      completion: (repost) {
        stopLoading();
        ...
        update();
        refreshView();
        showSnackBar(LKeys.repostSuccess.tr, type: SnackBarType.success);
      },
      onError: () {
        stopLoading();
        showSnackBar(LKeys.alreadyReposted.tr, type: SnackBarType.error);
      },
    );
  }
