...
class ImageVideoOptionPicker extends StatelessWidget {
  ...
  const ImageVideoOptionPicker({super.key, required this.onVideoTap, required this.onImageTap, this.onDocumentTap});
            ...
            child: GestureDetector(
              onTap: () async {
                if (widget.type == MessageType.image) {
                  ...
                  // WhatsApp/Telegram: compress image to WebP before sending
                  final compressed = await MediaCompressor.shared.compressImage(widget.file);
                  PostService.shared.uploadFile(compressed, (url) {
                  ...
                  widget.controller.startLoading();
                  // WhatsApp/Telegram: compress video before sending
                  final compressedVideo = await MediaCompressor.shared.compressVideo(widget.file);
                  PostService.shared.uploadFile(compressedVideo, (videoURL) {
                    if (thumbnail != null) {
                      PostService.shared.uploadFile(thumbnail!, (thumbURL) {
                        widget.controller.stopLoading();
                        Get.back();
                        widget.controller.commonSend(type: MessageType.video, content: videoURL, thumbnail: thumbURL);
                      });
                    }
                  });
                }
