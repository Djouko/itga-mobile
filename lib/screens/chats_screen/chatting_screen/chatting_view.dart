  ...
  Widget bottom(ChattingController controller) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: cWhite,
        border:
            Border(top: BorderSide(color: cLightText.withValues(alpha: 0.08))),
      ),
      child: SafeArea(
        top: false,
        child: Row(
                      ...
                      child: DetectableTextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintStyle: MyTextStyle.gilroyRegular(
                              color: cLightText.withValues(alpha: 0.6),
                              size: 15),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: cNavy.withValues(alpha: 0.25),
                      blurRadius: 8,
                      offset: const Offset(0, 2),
                    ),
                  ],
                ),
                child: const Icon(Icons.send_rounded, color: cWhite, size: 20),
              ),
            ),
          ],
        ),
      ...
      onTap: () {
        ...
        Get.bottomSheet(ImageVideoOptionPicker(
          onImageTap: () async {
            ...
            Loggers.info('Chat image picked: ${file?.path ?? 'none'}');
            if (file != null) {
              Get.back();
              Get.bottomSheet(
          ...
          onVideoTap: () async {
            ...
            Loggers.info('Chat video picked: ${file?.path ?? 'none'}');
            if (file != null) {
              Get.back();
              Get.bottomSheet(
                  WriteDescriptionSheet(
          ...
          children: [
            ...
            Expanded(
              child: GestureDetector(
                onTap: () {
                  if (controller.chatUserRoom?.profileType == 'company' &&
                      (controller.chatUserRoom?.companyId ?? 0) > 0) {
                    Get.to(() => CompanyPublicProfileScreen(
                          companyId:
                              controller.chatUserRoom!.companyId!.toInt(),
                        ));
            ...
            // Video call button: only for 1-on-1 chats with mutual follow (WhatsApp pattern)
            if (controller.chatUserRoom?.type != 2 &&
                controller.user != null &&
                controller.user?.followingStatus == 3)
              GestureDetector(
                onTap: () {
                  Get.to(() => VideoCallScreen(remoteUser: controller.user));
                },
                child: Container(
