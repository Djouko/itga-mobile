  ...
  void uploadPost() async {
    ...
    SightEngineService.shared.chooseTextModeration(
        ...
        completion: () async {
          ...
          // Instagram/WhatsApp pattern: compress all media before upload
          List<XFile> compressedImages = imageFileList;
          XFile? compressedVideo = videoFile;

          if (imageFileList.isNotEmpty) {
            compressedImages = await MediaCompressor.shared.compressImages(imageFileList);
          }
          if (videoFile != null) {
            compressedVideo = await MediaCompressor.shared.compressVideo(videoFile!);
          }
          ...
          PostService.shared.uploadPost(
            ...
            desc: InputSanitizer.sanitizeText(textEditingController.text),
            waves: waves,
            interestIds: selectedInterests.map((element) => '${element.id}').toList().join(','),
            mentionedUserIds: mentionedUsers.map((u) => '${u.id}').toList().join(','),
